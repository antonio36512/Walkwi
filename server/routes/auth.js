...
router.get('/me', authenticate, async (req, res) => res.json({ user: serializeUser(req.currentUser) }));

async function getAccountRestriction(user) {
  ...
  if (user.accountStatus === 'blocked') return { code: 'ACCOUNT_BLOCKED', error: 'Tu cuenta esta bloqueada.', reason: user.sanction?.reason };
  if (user.accountStatus === 'suspended') return { code: 'ACCOUNT_SUSPENDED', error: 'Tu cuenta esta suspendida.', reason: user.sanction?.reason, until: user.sanction?.until };
  return null;
}
...
router.patch('/me/profile', authenticate, async (req, res) => {
  try {
    ...
    if (req.currentUser?.role === 'walker' && pricePerHour !== undefined && Number(pricePerHour) <= 0) return res.status(400).json({ error: 'El precio por hora debe ser mayor que cero.' });
...
router.patch('/me/change-password', authenticate, async (req, res) => {
  try {
    ...
    const restriction = await getAccountRestriction(user);
...
router.get('/google/callback', async (req, res) => {
  try {
    ...
    const restriction = await getAccountRestriction(user);
    if (restriction) {
      const encoded = encodeURIComponent(JSON.stringify(restriction));
      return res.redirect(`${expoRedirect}/--/auth?restriction=${encoded}`);
    }
