...
function Agenda() {
  ...
  const fetchBookings = async () => {
    try {
      ...
      setBookings(data);
      if (role === 'user' && activeTab === 'completed') {
        const reviewData = await apiRequest('/api/reviews/mine');
        setReviewedBookingIds(reviewData.bookingIds || []);
      }
    } catch (err) {
      console.error(err);
    } finally {
  ...
  return (
    <View style={styles.screen}>
              ...
              return (
                ...
                >
                  ...
                  {booking.status === 'completed' && (
                    <View style={styles.actionsRow}>
                      {role === 'user' && !reviewedBookingIds.includes(booking._id) && <Pressable onPress={()=>{setReviewBooking(booking);setReviewRating(0);setReviewComment('');setReviewModalVisible(true)}} style={styles.reviewButton}><Ionicons name="star-outline" size={18} color="#fff"/><Text style={styles.reviewButtonText}>Calificar</Text></Pressable>}
                      <Pressable
      ...
      <Modal visible={reviewModalVisible} transparent animationType="slide"><Pressable style={styles.modalOverlay} onPress={()=>!reviewLoading&&setReviewModalVisible(false)}><Pressable style={styles.modalSheet} onPress={e=>e.stopPropagation()}><View style={styles.modalHandle}/><Text style={styles.modalTitle}>Calificar servicio</Text><Text style={styles.modalSubtitle}>Comparte tu experiencia con el paseador</Text><View style={styles.starsRow}>{[1,2,3,4,5].map(star=><Pressable key={star} onPress={()=>setReviewRating(star)}><Ionicons name={star<=reviewRating?'star':'star-outline'} size={36} color="#f59e0b"/></Pressable>)}</View><TextInput multiline value={reviewComment} onChangeText={setReviewComment} placeholder="Escribe tu reseña" placeholderTextColor="#8fa899" style={styles.modalInput}/><View style={styles.modalActions}><Pressable style={styles.modalCancelButton} onPress={()=>setReviewModalVisible(false)}><Text style={styles.modalCancelText}>Cancelar</Text></Pressable><Pressable disabled={!reviewRating||reviewComment.trim().length<3||reviewLoading} style={[styles.modalConfirmButtonGreen,(!reviewRating||reviewComment.trim().length<3)&&styles.modalConfirmDisabled]} onPress={async()=>{try{setReviewLoading(true);await apiRequest('/api/reviews',{method:'POST',body:JSON.stringify({bookingId:reviewBooking._id,rating:reviewRating,comment:reviewComment.trim()})});setReviewedBookingIds(ids=>[...ids,reviewBooking._id]);setReviewModalVisible(false);alert('Reseña publicada.')}catch(e){alert(e.message)}finally{setReviewLoading(false)}}}><Text style={styles.modalConfirmText}>{reviewLoading?'Publicando...':'Publicar'}</Text></Pressable></View></Pressable></Pressable></Modal>
...
const styles = StyleSheet.create({
  ...
  reviewButton: { alignItems:'center', backgroundColor:colors.primary, borderRadius:16, flex:1, flexDirection:'row', gap:6, justifyContent:'center', paddingVertical:12 },
  reviewButtonText: { color:'#ffffff', fontSize:14, fontWeight:'900' },
  starsRow: { flexDirection:'row', gap:8, justifyContent:'center', marginVertical:20 },
  reportButtonText: {
    color: '#92400e',
    fontSize: 14,
    fontWeight: '900',
