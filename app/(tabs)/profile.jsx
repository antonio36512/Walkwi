...
function Profile() {
  ...
  const saveProfile = async () => {
    ...
    if (profile.role === 'walker' && (!Number.isFinite(Number(draftProfile.pricePerHour)) || Number(draftProfile.pricePerHour) <= 0)) {
      setProfileError('Ingresa un precio por hora mayor que cero.');
      return;
    }
    ...
    try {
      ...
      const data = await apiRequest('/api/auth/me/profile', {
        ...
        body: JSON.stringify({
          ...
          pricePerHour: draftProfile.pricePerHour,
          ...
          {profile.role === 'walker' ? (
            <>
              ...
              <TextInput
                ...
                onChangeText={(value) => updateDraftProfile('pricePerHour', value.replace(/[^0-9.]/g, ''))}
                placeholder="Ej. 12.50"
                placeholderTextColor="#8fa899"
                style={styles.input}
                value={draftProfile.pricePerHour}
              />
